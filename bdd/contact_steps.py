...
def new_contact(app, last_name, first_name, address):
    return Contact(last_name=last_name, first_name=first_name, address=address)
...
def verify_contact_edit(db, contact_list, new_contact):
...
def non_empty_contact_list(app, db):
    if len(db.get_contact_list()) == 0:
        app.contact.create(Contact(last_name="Contact for delete"))
    return db.get_contact_list()
